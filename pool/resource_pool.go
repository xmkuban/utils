...
func (rp *ResourcePool) get(ctx context.Context, wait bool) (resource Resource, err error) {
	...
	rp.bannedTagsMutex.RLock()
	if len(rp.bannedTags) > 0 && wrapper.resource != nil && utils.KeyInSlice(wrapper.resource.Tag(), rp.bannedTags) >= 0 {
		rp.bannedTagsMutex.RUnlock()
		wrapper.resource.Close()
		wrapper.resource = nil
	} else {
		rp.bannedTagsMutex.RUnlock()
...
func (rp *ResourcePool) Put(resource Resource) {
	...
	if resource != nil {
		rp.bannedTagsMutex.RLock()
		if len(rp.bannedTags) > 0 && utils.KeyInSlice(resource.Tag(), rp.bannedTags) >= 0 {
			rp.bannedTagsMutex.RUnlock()
			resource.Close()
		} else {
			rp.bannedTagsMutex.RUnlock()
...
// Remove banned tag
func (rp *ResourcePool) RemoveBannedTag(tag string) {
	rp.bannedTagsMutex.Lock()
	for i, t := range rp.bannedTags {
		if t == tag {
			rp.bannedTags = append(rp.bannedTags[:i], rp.bannedTags[i+1:]...)
			break
		}
	}
	rp.bannedTagsMutex.Unlock()
}

// Add banned tag
func (rp *ResourcePool) AddBannedTag(tag string) {
	if tag == "" {
		return
	}
	rp.bannedTagsMutex.Lock()
	if utils.KeyInSlice(tag, rp.bannedTags) >= 0 {
		return
	}
	rp.bannedTags = append(rp.bannedTags, tag)
	rp.bannedTagsMutex.Unlock()
}

// Is in banned tag
func (rp *ResourcePool) IsBannedTag(tag string) bool {
	if tag == "" {
		return false
	}
